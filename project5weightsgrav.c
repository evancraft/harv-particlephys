...
TH1F * graphmmt(const char* filename, const char* branchname, const char* branchname2, const char* branchname3, const char* branchname4, const char* branchname5, const char* branchname6, const char* branchname7, const char* branchname8, const char* branchname9, const char* title, int bins, int lower, int upper, short color, bool weighted, int cat){
	...
	//hist -> Fill(mass/10000, 1);
	hist -> Fill(mass/1000, .076);
	} else if (cat == 3) {
	//hist -> Fill(mass/10000, .000005);
	hist -> Fill(mass/1000, 2.5);
...
void project5weightsgrav(){

	TH1F *dilepton = 0;
	TH1F *monte = 0;
	TH1F *data = 0;
	
	dilepton = graphmmt("mc.graviton.4lep.root", "lep_pt", "lep_E", "lep_phi", "lep_eta", "scaleFactor_MUON", "scaleFactor_ELE", "scaleFactor_PILEUP", "scaleFactor_LepTRIGGER", "mcWeight", "Inv. Mass", 50, 0, 800, kRed-9, true, 1);

	monte = graphmmt("mc.graviton.4lep.root", "lep_pt", "lep_E", "lep_phi", "lep_eta", "scaleFactor_MUON", "scaleFactor_ELE", "scaleFactor_PILEUP", "scaleFactor_LepTRIGGER", "mcWeight", "Inv. Mass", 50, 0, 800, kBlue-9, false, 2);

	//data = graphmmt("data.4lep.root", "lep_pt", "lep_E", "lep_phi", "lep_eta", "scaleFactor_MUON", "scaleFactor_ELE", "scaleFactor_PILEUP", "scaleFactor_LepTRIGGER", "mcWeight", "Inv. Mass", 50, 0, 200, kBlack, false, 3);
   ...
   c1->cd(1);
	...
	//data -> Draw("HIST");
   hs1->Draw("hist");
   hs1->GetHistogram()->GetYaxis()->SetTickLength(0);
   hs1->GetHistogram()->GetYaxis()->SetLabelOffset(999);
   hs1->GetHistogram()->GetXaxis()->SetTitle("#it{m} [GeV]");
   hs1->GetHistogram()->GetYaxis()->SetTitle("Events");
	//data->Draw("SAME");
	//data->SetMarkerStyle(10);

	TLegend *leg = new TLegend(0.7, 0.5, 0.9, 0.7);
	leg -> AddEntry(dilepton, "Graviton (MC)", "f");
