...
void ratioplot(TH1F *h1, TH1F *h2) {
   gStyle->SetOptStat(0);
   auto c1 = new TCanvas("c1", "A ratio example");
   h1->Sumw2();
   h2->Scale(.076);
	h1 -> GetXaxis() -> SetTitle("#it{m} [GeV]");
...
void project5weightsratio(){

	TH1F *dilepton = 0;
	TH1F *monte = 0;
	TH1F *data = 0;
	
	dilepton = graphmmt("mc.graviton.4lep.root", "lep_pt", "lep_E", "lep_phi", "lep_eta", "scaleFactor_MUON", "scaleFactor_ELE", "scaleFactor_PILEUP", "scaleFactor_LepTRIGGER", "mcWeight", "Inv. Mass", 50, 0, 800, kBlue-20, true, 1);
	...
	ratioplot(dilepton, monte);

	//TLegend *leg = new TLegend(0.7, 0.5, 0.9, 0.7);
	//leg -> AddEntry(dilepton, "Gravtiton (MC)", "f");
	//leg -> AddEntry(monte, "Ratio", "f");
